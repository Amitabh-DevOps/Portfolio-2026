    ...
    return (
        ...
        >
            <div className="relative aspect-video w-full rounded-xl overflow-hidden mb-5 bg-slate-950/70">
                ...
                <a href={link} target="_blank" rel="noopener noreferrer" onPointerDown={(event) => event.stopPropagation()} onMouseDown={(event) => event.stopPropagation()} onTouchStart={(event) => event.stopPropagation()} onClick={(event) => event.stopPropagation()} aria-label={`View ${title} repository`} className={`absolute top-3 right-3 p-3 bg-slate-950/75 border border-white/15 rounded-lg transition-all duration-300 hover:bg-primary hover:text-slate-950 hover:border-primary text-white z-20 ${isActive ? "opacity-100 translate-y-0 pointer-events-auto" : "opacity-0 translate-y-1 pointer-events-none group-hover:opacity-100 group-hover:translate-y-0 group-hover:pointer-events-auto focus-visible:opacity-100 focus-visible:translate-y-0 focus-visible:pointer-events-auto"}`}>
